...
void Manifold_Base::createRandomPoint(RefVec out, double coeff) const
{
  mnf_assert(out.size() == representationDim_ &&
             "wrong dimension in Manifold_Base::createRandomPoint");
  createRandomPoint_(out, coeff);
}

Index Manifold_Base::dim() const { return dimension_; }

Index Manifold_Base::tangentDim() const { return tangentDim_; }

Index Manifold_Base::representationDim() const { return representationDim_; }
...
void Manifold_Base::retractation(RefVec out, const ConstRefVec& x,
...
void Manifold_Base::pseudoLog(RefVec out, const ConstRefVec& x,
...
void Manifold_Base::applyDiffRetractation(RefMat out, const ConstRefMat& in,
...
void Manifold_Base::applyDiffPseudoLog0(RefMat out, const ConstRefMat& in,
                                        const ConstRefVec& x) const
{
...
void Manifold_Base::applyTransport(RefMat out, const ConstRefMat& in,
                                   const ConstRefVec& x,
                                   const ConstRefVec& v) const
{
...
void Manifold_Base::applyInvTransport(RefMat out, const ConstRefMat& in,
                                      const ConstRefVec& x,
                                      const ConstRefVec& v) const
{
  mnf_assert(in.rows() == tangentDim_);
...
void Manifold_Base::applyInvTransportOnTheRight(RefMat out,
...
bool Manifold_Base::isInTxM(const ConstRefVec& x, const ConstRefVec& v,
...
void Manifold_Base::forceOnTxM(RefVec out, const ConstRefVec& in,
                               const ConstRefVec& x) const
{
  mnf_assert(out.size() == tangentDim_);
  mnf_assert(x.size() == representationDim());
...
void Manifold_Base::lock() const { lock_ = true; }
...
bool Manifold_Base::isLocked() const { return lock_; }

const std::string& Manifold_Base::name() const { return name_; }

std::string& Manifold_Base::name() { return name_; }

long Manifold_Base::getInstanceId() const { return this->instanceId_; }
