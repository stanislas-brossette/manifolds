...
S2_Base::S2_Base(double magnitude) : Manifold_Base(2, 3, 3)
{
  name() = "S2";
  setTypicalMagnitude(Eigen::Vector3d::Constant(magnitude));
  setTrustMagnitude(Eigen::Vector3d::Constant(magnitude));
}
S2_Base::S2_Base(const ConstRefVec& magnitude) : Manifold_Base(2, 3, 3)
{
...
S2_Base S2_Base::copy() const { return S2_Base(*this); }
...
size_t S2_Base::numberOfSubmanifolds() const { return 1; }
...
std::string S2_Base::toString(const ConstRefVec& val, const std::string& prefix,
...
void S2_Base::createRandomPoint_(RefVec out, double) const { rand(out); }

void S2_Base::retractation_(RefVec out, const ConstRefVec& x,
...
void S2_Base::pseudoLog_(RefVec out, const ConstRefVec& x,
                         const ConstRefVec& y) const
{
  logarithm(out, x, y);
}

void S2_Base::logarithm(RefVec out, const ConstRefVec& x,
                        const ConstRefVec& y) const
{
...
void S2_Base::projRows(RefMat out, const ConstRefMat& in,
                       const ConstRefVec& x) const
{
  for (Index i = 0; i < in.rows(); ++i)
    out.row(i) = in.row(i) - (x * in.row(i)).trace() * x.transpose();
}
void S2_Base::projCols(RefMat out, const ConstRefMat& in,
                       const ConstRefVec& x) const
{
  for (Index i = 0; i < in.cols(); ++i) projVec(out.col(i), in.col(i), x);
}
void S2_Base::projVec(RefVec out, const ConstRefVec& in,
                      const ConstRefVec& x) const
{
  out = in - (x.dot(in)) * x;
}
...
void S2_Base::applyDiffRetractation_(RefMat out, const ConstRefMat& in,
                                     const ConstRefVec& x) const
{
  projRows(out, in, x);
}
...
void S2_Base::applyDiffPseudoLog0_(RefMat, const ConstRefMat&,
                                   const ConstRefVec&) const
{
  throw std::runtime_error("Unimplemented method in S2");
}

void S2_Base::applyTransport_(RefMat out, const ConstRefMat& in,
...
void S2_Base::applyInvTransport_(RefMat out, const ConstRefMat& in,
                                 const ConstRefVec& x,
                                 const ConstRefVec& v) const
{
  Eigen::Vector3d y;
...
void S2_Base::applyInvTransportOnTheRight_(RefMat out, const ConstRefMat& in,
                                           const ConstRefVec& x,
                                           const ConstRefVec& v) const
{
  Eigen::Vector3d y;
...
bool S2_Base::isInTxM_(const ConstRefVec& x, const ConstRefVec& v,
...
void S2_Base::forceOnTxM_(RefVec out, const ConstRefVec& v,
                          const ConstRefVec& x) const
{
  out = v - x.dot(v) * x;
}
