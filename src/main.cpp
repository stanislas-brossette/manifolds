...
int main()
{
  ...
  CartesianProduct R3SO3R3(R3, RotSpace);
  R3SO3R3.multiply(R3);
  {
  ...
  }
  {
    ...
    Eigen::VectorXd v(9);
    Eigen::MatrixXd M(9,4);
    M = Eigen::MatrixXd::Random(9,4);
    Eigen::MatrixXd MTrans(9,4);
    v << 1,2,3,0,0,0,4,5,6;
    Point x = R3SO3R3.getIdentity();
    x.increment(v);
    R3SO3R3.applyTransport(MTrans, M, v);
    std::cout << "v = " << std::endl<< v << std::endl;
    std::cout << "M = "<< std::endl << M << std::endl;
    std::cout << "CardProdSpace.applyTransport(MTrans, M, v) = "<< std::endl<< MTrans << std::endl;
  }
