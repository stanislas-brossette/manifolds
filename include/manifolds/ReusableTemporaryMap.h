...
class MANIFOLDS_API ReusableTemporaryMap
{
 public:
  ...
  Eigen::Map<Eigen::MatrixXd, Eigen::Aligned> getMap(Eigen::DenseIndex m,
                                                     Eigen::DenseIndex n);

 private:
  ReusableTemporaryMap& operator=(
      const ReusableTemporaryMap&);  // We forbid copy
...
inline Eigen::Map<Eigen::MatrixXd, Eigen::Aligned> ReusableTemporaryMap::getMap(
    Eigen::DenseIndex m, Eigen::DenseIndex n)
{
