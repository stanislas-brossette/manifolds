...
constexpr uint computeK_1(uint k) { return k * c1; }
constexpr uint computeK_2(uint k) { return (k << r1) | (k >> (32 - r1)); }
constexpr uint computeK_3(uint k) { return k * c2; }
constexpr uint computeRound_1(uint hash, uint k) { return hash ^ k; }
constexpr uint computeRound_2(uint hash)
{
  return (hash << r2) | (hash >> (32 - r2));
}
constexpr uint computeRound_3(uint hash) { return hash * m + n; }
...
constexpr uint finalize_1(uint hash) { return hash ^ (hash >> 16); }
constexpr uint finalize_2(uint hash) { return hash * 0x85ebca6b; }
constexpr uint finalize_3(uint hash) { return hash ^ (hash >> 13); }
constexpr uint finalize_4(uint hash) { return hash * 0xc2b2ae35; }
  ...
  return (index > N + 3
              ...
              : computeHash_recurse(
                    ...
                    computeRound(
                        hash,
                        assembleBytes(
                            static_cast<uint8_t>(index < N ? str[index] : 0),
                            static_cast<uint8_t>(index + 1 < N ? str[index + 1]
                                                               : 0),
                            static_cast<uint8_t>(index + 2 < N ? str[index + 2]
                                                               : 0),
                            static_cast<uint8_t>(index + 3 < N ? str[index + 3]
                                                               : 0)))));
}
...
#else
template <int N>
constexpr uint computeHash_recurse(const char(&str)[N], int index = 0,
                                   uint hash = 0)
{
  return (index > N + 3
              ? hash
