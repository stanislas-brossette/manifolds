...
class MANIFOLDS_API ConstSubPoint : public ConstManifoldOwner
{
 ...
 protected:
  ...
  /// SubPoint.
  /// This require a const_cast upon building ConstSubPoint, however we honor
  /// the
...
class MANIFOLDS_API SubPoint : public ConstSubPoint
{
 ...
 public:
  ...
  {
    return ConstSubPoint::operator()(i);
  }
...
class MANIFOLDS_API Point : public PointMemory, public SubPoint
{
 ...
 public:
  ...
  /// \internal For now, we keep operations on Point only. ConstSubPoint and
  /// SubPoint
  ...
  /// \brief Fills vector out with the value of a point that is the result of a
  /// retractation of v
  /// at the current point x. \f$ out = \phi_x(v) \f$
  void retractation(RefVec out, const ConstRefVec& v) const;
  /// \brief Fills point out with the value of a point that is the result of a
  /// retractation of v
  /// at the current point x. \f$ out = \phi_x(v) \f$
  void retractation(Point& out, const ConstRefVec& v) const;

  /// \brief Computes a vector that is the pseudoLog of point y at the current
  /// point x
  /// \f$ out = Log_x(y) \f$
  Eigen::VectorXd pseudoLog(const Point& y) const;
  /// \brief Fills vector out with a vector that is the pseudoLog of point y at
  /// the current point x
  /// \f$ out = Log_x(y) \f$
  void pseudoLog(RefVec out, const Point& y) const;

  /// \brief Computes a vector that is the pseudoLog0 of current point x at
  /// point 0
  /// \f$ out = Log_0(x) \f$
  ...
  /// \brief Checks that the current point's value represents a valid point of
  /// its Manifold
  ...
  void applyTransport(RefMat out, const ConstRefMat& in,
